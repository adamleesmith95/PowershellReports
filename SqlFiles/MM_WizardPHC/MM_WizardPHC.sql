SELECT DISTINCT
    ...
    sph.update_date AS Update_Date,
    sdrpp.expiration_date AS Max_PHC_Date
FROM 
    s_product_header_mm_function sphmf
    JOIN s_product_header sph ON sphmf.product_header_code = sph.product_header_code
    JOIN s_display_category sdc ON sph.display_category_code = sdc.display_category_code
    JOIN s_system_function ssf ON sphmf.mm_function_code = ssf.system_function_code
    JOIN s_date_range_product_price sdrpp ON sph.product_header_code = sdrpp.product_header_code 
WHERE
sph.active_ind = 'Y'
AND sph.display_ind = 'Y'      
AND sdc.active_ind = 'Y'
AND sph.price_by_season_ind = 'N'
AND sph.update_date <= DATEADD(DAY, -557, GETDATE())
AND sdrpp.expiration_date >= GETDATE()
...
AND sph.update_date <= GETDATE() - 557
