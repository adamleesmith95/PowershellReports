...
# Get the row count from the first worksheet (assuming both worksheets have the same number of rows)
$rowCount = $worksheet1.Count
...
for ($i = 0; $i -lt $rowCount; $i++) {
    ...
    # Initialize a custom object to store the row data from the second file
    ...
    foreach ($column in $worksheet2 | Get-Member -MemberType Properties | Where-Object { $_.Name -ne "PSObject" }) {
        ...
        $result | Add-Member -MemberType NoteProperty -Name $columnName -Value $row2.$columnName
    }

    # Iterate through all columns in the first file
    foreach ($column in $worksheet1 | Get-Member -MemberType Properties | Where-Object { $_.Name -ne "PSObject" }) {
        $columnName = $column.Name
        ...
        if ($value1 -ne $value2) {
            # Add the "Original" column name and value from the first file
            $originalColumnName = "Original" + $columnName

            # Check if the member already exists and handle it accordingly
            if ($result | Get-Member -Name $originalColumnName -MemberType Properties) {
                # Append the value to the existing member
                $result.$originalColumnName += ", " + $value1
            } else {
                # Create a new member
                $result | Add-Member -MemberType NoteProperty -Name $originalColumnName -Value $value1
            }
        }
    }

    # Special case for the "PricingSeason" column
    if ($row1.PricingSeason -ne $row2.PricingSeason) {
        $originalPricingSeason = "OriginalPricingSeason"

        # Check if the member already exists and handle it accordingly
        if ($result | Get-Member -Name $originalPricingSeason -MemberType Properties) {
            # Append the value to the existing member
            $result.$originalPricingSeason += ", " + $row1.PricingSeason
        } else {
            # Create a new member
            $result | Add-Member -MemberType NoteProperty -Name $originalPricingSeason -Value $row1.PricingSeason
        }
    }
